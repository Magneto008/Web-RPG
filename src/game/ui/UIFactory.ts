...
export class UIFactory {
  /**
   * Creates a simple dialog box with a background image, title, and content text.
   */
  static createDialog(scene: Phaser.Scene, config: {
    x: number,
    y: number,
    width?: number,
    height?: number,
  ...
  /**
   * Creates a menu with a list of options.
   */
  static createScrollablePanel(scene: Phaser.Scene, config: {
    x: number,
    y: number,
    width: number,
    height: number,
    ...
    const sizer = panel.getElement('panel') as any;
    config.items.forEach(item => {
      const label = rexUI.add.label({
        background: scene.add.image(0, 0, config.backgroundKey).setDisplaySize(config.width - 40, 40),
